...
export async function getSvgLocal(
  ...
  if (!built) return;
  const xlink = built.body.includes("xlink:")
    ? ' xmlns:xlink="http://www.w3.org/1999/xlink"'
    : "";
  return `<svg xmlns="http://www.w3.org/2000/svg"${xlink} ${Object.entries(
    built.attributes,
  )
    .map(([k, v]) => `${k}="${v}"`)
...
export async function getSvg(
  ...
  return (
    ...
    (await fetch(
      `${API_ENTRY}/${icon}.svg?inline=false&height=${size}&color=${encodeURIComponent(color)}`,
    ).then((r) => r.text())) ||
    ""
  );
}
...
export async function getSvgSymbol(
  ...
  const symbolElem = document.createElementNS(
  ...
  symbolElem.setAttribute(
    "viewBox",
    node.querySelector("svg")!.getAttribute("viewBox")!,
  );
...
export function ClearSvg(svgCode: string, reactJSX?: boolean) {
  ...
  for (const key of Object.values(svg.attributes)) {
    if (keep.includes(key.localName)) continue;
...
export function ${name}(props) {
  ...
  if (snippet) return prettierCode(code, "babel-ts");
  else
    return prettierCode(
...
export default {
  ...
  const code = isTs
    ? content.replace("<script>", '<script lang="ts">')
    : content;
...
export function SvgToReactNative(svg: string, name: string, snippet: boolean) {
  function replaceTags(
    ...
    replacements.forEach(({ from, to }) => {
      result = result
        .replace(new RegExp(`<${from}(.*?)>`, "g"), `<${to}$1>`)
  ...
  function generateImports(usedComponents: string[]): string {
    ...
    if (svgIndex !== -1) usedComponents.splice(svgIndex, 1);
  ...
  const imports = generateImports(
    svgComponents.filter((component) => reactNativeSvgCode.includes(component)),
  );
...
export async function getIconSnippet(
  ...
  switch (type) {
    ...
    case "jsx":
      return SvgToJSX(
        await getSvg(icon, undefined, color),
